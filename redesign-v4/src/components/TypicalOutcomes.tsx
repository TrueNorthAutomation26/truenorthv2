...
export function TypicalOutcomes() {
  return (
    <section className="py-28 lg:py-36 relative border-t border-white/10">
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        <div className="text-center max-w-2xl mx-auto mb-16">
          <AnimateIn>
            <div className="flex justify-center">
              <Badge variant="teal">What month one typically looks like</Badge>
            </div>
          </AnimateIn>
          <AnimateIn delay={0.1}>
        ...
        >
          {outcomes.map((o) => (
            <StaggerItem key={o.headline}>
              <div className="group panel-dark panel-dark-hover relative rounded-2xl p-8 h-full flex flex-col">
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <span className="font-mono text-xs tracking-widest uppercase text-white/35">
                    {o.industry}
                  </span>
                </div>
                ...
                <div className="mt-6 pb-6 border-b border-white/10">
                  <p className="text-3xl lg:text-4xl font-extrabold text-gold font-mono tracking-tight">
                    {o.metric}
                  </p>
                  <p className="mt-1 text-sm text-white/40">{o.metricLabel}</p>
                </div>
        ...
        <AnimateIn delay={0.3}>
          <div className="mt-10 text-center">
            <Link
              href="/results"
              className="inline-flex items-center gap-2 text-teal-light font-semibold hover:gap-3 transition-all duration-200 cursor-pointer"
            >
              See full scenarios with month-by-month math
              <ArrowRight className="w-4 h-4" />
            </Link>
          </div>
          <p className="mt-8 text-center text-xs text-white/30 italic max-w-2xl mx-auto">
